...
for (i in 28000) { #seq(from = range_pred[1], to = range_pred[2], by = 50)) {
  ...
  df_pred <-  df_merge %>% 
    ...
    mutate(DistanceAlignment = 0) %>%
    mutate(DistanceCrown = 3.048)
  
  df_pred_initial <- df_pred
  for (j in seq(from = 0.5, to = 20, by = 0.5)) {
    df_pred_j <- df_pred_initial %>% mutate(DistanceAlignment = j)
    df_pred <- rbind(df_pred, df_pred_j)
  }
  ...
  result <- data.frame(ChainageHead = df_pred$ChainageHead,
                       ...
                       DistanceAlignment = df_pred$DistanceAlignment,
